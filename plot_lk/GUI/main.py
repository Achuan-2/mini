...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ui.color_picker1.clicked.connect(partial(self.color_picker,self.ui.color1))
        self.ui.color_picker2.clicked.connect(partial(self.color_picker,self.ui.color2))
        self.ui.color_picker3.clicked.connect(partial(self.color_picker,self.ui.color3))
    def setup(self):
    ...
    def file_choose(self):
        ...
        self.ui.plot_button.setEnabled(False)
        self.ui.save_button.setEnabled(False)
        input_file=QFileDialog.getOpenFileName(QMainWindow(), "Choose File")[0]
        if input_file:
    ...
    def plot_graph(self,time_start=0, time_end=60, y_min=0,
        ...
        # deal with df_plot
        try:
            df_plot = self.data.df[(self.data.df["min"] > time_start) & (self.data.df["min"] < time_end)].copy()
        except:
        ...
        del df_plot
    def save_plot(self):
        file_name, _ = QFileDialog.getSaveFileName(QMainWindow(), "保存图片", "", "PNG(*.png);;JPG(*.jpg);;BMP(*.bmp);;TIFF(*.tif);;PDF(*.pdf)")
        if file_name:
